    ...
    <main className="min-h-screen bg-background text-on-surface py-8 md:py-12">
      <div className="max-w-[1280px] mx-auto px-4 md:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
          ...
          <Link
            href="/jobs/browse"
            className="inline-flex items-center justify-center gap-2 bg-primary text-on-primary px-5 py-2.5 rounded-lg text-sm font-semibold hover:bg-primary-container transition-all cursor-pointer w-fit"
          >
                    ...
                    <div className="space-y-2 min-w-0">
                      ...
                      {job && (
                        <div className="text-xs text-secondary flex items-center gap-1.5 truncate">
                          ...
                          <span>{getShortAddress(job.dropoffAddress)}</span>
                        </div>
                      )}

                      <div className="text-xs text-secondary flex items-center gap-2">
                        <span>Created: {formatCompletedDate(payout.createdAt)}</span>
                        {payout.paidAt && (
                          <span>· Paid: {formatCompletedDate(payout.paidAt)}</span>
                        )}
                      </div>

                      {payout.notes && (
                        <p className="text-xs text-secondary bg-surface-container-low p-2 rounded max-w-lg">
                          <span className="font-semibold">Note:</span> {payout.notes}
                        </p>
                      )}
                    </div>

                    {/* Right: Amounts */}
                    <div className="text-left md:text-right shrink-0 flex flex-col justify-center">
                      <p className="text-xs text-secondary uppercase font-semibold tracking-wider">
                        Driver Payout
                      </p>
                      ...
                      {payout.platformFee > 0 && (
                        <p className="text-xs text-secondary mt-0.5">
                          Platform Fee: NPR {payout.platformFee.toLocaleString("en-NP")}
                        </p>
                      )}
                    </div>
                  </div>
