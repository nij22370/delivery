...
async function fetchDeliveredJobs(): Promise<JobsApiResponse> {
  ...
  if (!response.ok) throw new Error("Failed to load delivered jobs");
  return response.json() as Promise<JobsApiResponse>;
}

function buildSpendingData(jobs: BillingJob[]): SpendingPoint[] {
  return [...jobs]
    .sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime())
    .map((job) => ({
      label: formatShortDate(job.createdAt),
...
export default function BillingPage() {
  ...
  const spendingData = useMemo(() => buildSpendingData(jobs), [jobs]);
          ...
          <div className="h-48 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={spendingData} margin={{ top: 5, right: 8, left: 0, bottom: 5 }}>
                ...
                <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: "#424654" }} allowDecimals={false} width={40} />
