...
const ADMIN_PAYOUTS_ENDPOINT = "/api/admin/payouts";
...
async function fetchAdminJobs(): Promise<AdminJobsResponse> {
  const params = new URLSearchParams({
    limit: String(PAGE_SIZE * 5),
    page: "1",
  });
  const response = await apiFetch(`${ADMIN_JOBS_ENDPOINT}?${params}`);
  if (!response.ok) throw new Error("Failed to load admin jobs");
  return response.json() as Promise<AdminJobsResponse>;
}

async function fetchAdminPayouts(): Promise<AdminPayoutsResponse> {
  const params = new URLSearchParams({
    limit: String(PAGE_SIZE * 5),
    page: "1",
  });
  const response = await apiFetch(`${ADMIN_PAYOUTS_ENDPOINT}?${params}`);
  if (!response.ok) throw new Error("Failed to load admin payouts");
  return response.json() as Promise<AdminPayoutsResponse>;
}
...
const PAYMENT_COLUMNS = [
  ...
  payColHelper.accessor("driverName", {
    header: "Driver",
    cell: (info) => <span className="text-secondary font-medium">{info.getValue()}</span>,
  }),
...
export default function AdminHistory() {
  ...
  const { data: adminJobsData, isLoading: isJobsLoading, isError: isJobsError } = useQuery<
    AdminJobsResponse,
    Error
  >({
    queryKey: ["history-admin-jobs"],
    queryFn: fetchAdminJobs,
    staleTime: 30_000,
  });

  const { data: adminPayoutsData, isLoading: isPayoutsLoading, isError: isPayoutsError } = useQuery<
    AdminPayoutsResponse,
    Error
  >({
    queryKey: ["history-admin-payouts"],
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold text-on-surface">History (Admin Control)</h1>
      ...
      {activeTab === "jobs" && (
        <>
          {isJobsError ? (
            <p className="text-error-red font-medium">Failed to load system job history.</p>
          ) : (
            <DataTableShell
              ...
              emptyIcon="inbox"
              emptyMessage="No system job history found."
            >
      ...
      {activeTab === "payments" && (
        <>
          {isPayoutsError ? (
            <p className="text-error-red font-medium">Failed to load system payouts.</p>
          ) : (
            <DataTableShell
              ...
              emptyMessage="No system payout records found."
            >
              <thead className="bg-surface-container-low text-secondary uppercase font-semibold text-xs border-b border-outline-variant">
