...
function PaymentStatusBadge({ status }: { status?: string }) {
  ...
  const displayStatus = rawStatus === "paid" ? "Received" : rawStatus;
  const className =
    rawStatus === "paid"
...
async function fetchAllDriverJobs(): Promise<JobsApiResponse> {
  ...
  if (!response.ok) throw new Error("Failed to load driver jobs");
...
const JOB_COLUMNS = [
  ...
  jobColHelper.display({
    ...
    cell: (info) => {
      ...
      const isActive = status === JOB_STATUS.ACCEPTED || status === JOB_STATUS.IN_TRANSIT;
...
const PAYMENT_COLUMNS = [
  ...
  payColHelper.accessor("status", {
    header: "Status",
    cell: (info) => (
      <PaymentStatusBadge status={info.getValue()} />
    ),
  }),
...
function mapPayoutToPaymentRecord(payout: DriverPayoutItem): PaymentRecord {
  const populatedJob =
    typeof payout.jobId !== "string" ? (payout.jobId as PopulatedPayoutJob) : null;
  const rawId =
    populatedJob?._id ?? (typeof payout.jobId === "string" ? payout.jobId : payout._id);
  return {
    jobId: rawId,
    displayJobId: rawId.slice(-6).toUpperCase(),
...
export default function DriverHistory() {
  ...
  const { data: driverAllJobs, isLoading: isJobsLoading, isError: isJobsError } = useQuery<
    JobsApiResponse,
    Error
  >({
    queryKey: ["history-driver-all-jobs"],
    queryFn: fetchAllDriverJobs,
    staleTime: 30_000,
  });

  const { data: payoutsData, isLoading: isPayoutsLoading } = useDriverPayouts(true);

  const jobTableRows = useMemo<JobsTableRow[]>(() => {
    const list = (driverAllJobs?.jobs ?? []).filter((job) =>
      DRIVER_HISTORY_STATUSES.includes(job.status as (typeof DRIVER_HISTORY_STATUSES)[number])
    );
    return list
  ...
  const paymentRecords = useMemo<PaymentRecord[]>(() => {
    return (payoutsData?.payouts ?? []).map(mapPayoutToPaymentRecord);
  }, [payoutsData?.payouts]);
        ...
        <>
          {paymentRecords.length > 0 && (
            <div className="bg-surface-container-low rounded-xl px-6 py-4 flex justify-between items-center">
              <p className="text-sm text-secondary font-medium">Total Earned</p>
              <p className="text-xl font-bold text-on-surface">NPR {totalEarned.toLocaleString("en-NP")}</p>
            </div>
          )}
          <DataTableShell
