...
**Last updated:** Aug 29 2026 — Admin dispute UX overhaul: simplified resolve modal (dropdowns → step-by-step radio buttons with split NPR inputs), added admin-to-user messaging panel (poster/driver tabs, Message model, Pusher trigger on private-job channel), replaced four ambiguous bottom buttons with Dismiss + Resolve Dispute. Created `GET/POST /api/jobs/[id]/admin-message` with `withRole(["admin"])` guard. Build: tsc 0 errors on changed files, ESLint 0 warnings. tsc has 2 pre-existing errors in `admin/users/page.tsx` (BUG-15/16 scope).

---

## Session Log

| Aug 29 | **Dispute UX Overhaul (3 fixes):** 1) Simplified `ResolveDisputeModal.tsx` — replaced two dropdowns with step-by-step radio buttons (Step 1: cancel/re-post, Step 2: refund/pay/split with NPR inputs, Step 3: note textarea), single "Confirm Resolution" button, removed `initialResolvedStatus`/`initialPayoutStatus` props and `modalPreset` state; 2) Added `AdminMessagePanel` (poster/driver tabs, scrollable history via `GET /api/jobs/:id/admin-message`, optimistic send via `POST`), placed below Evidence & Claims; removed non-functional chat bubble icons from Poster/Driver cards; 3) Replaced four bottom buttons (Dismiss/Split/Pay Driver/Refund Poster) with two: "Dismiss (no action needed)" (grey, direct resolve) and "Resolve Dispute →" (blue, opens modal). New API route: `GET/POST /api/jobs/[id]/admin-message` with `withRole(["admin"])`, reuses `Message` model and `triggerJobEvent` for Pusher `new-message` on `private-job-{jobId}` channel. tsc 0 errors on changed files, ESLint 0 warnings. Traced as BUG-17/18/19 in Bug.md. | `src/components/admin/ResolveDisputeModal.tsx` (rewritten), `src/components/admin/AdminMessagePanel.tsx` (new), `src/app/(admin)/admin/disputes/page.tsx` (modified), `src/app/api/jobs/[id]/admin-message/route.ts` (new) | None |
