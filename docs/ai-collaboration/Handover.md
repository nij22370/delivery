...
**Last updated:** Aug 30 2026 — Settings & Header UI cleanup complete: Change Password card styled for light theme (`bg-surface-white border border-outline-variant`), inputs styled with standard borders and light backgrounds, `ProfileSidebar` deleted (settings now renders as single centered card). Moved Settings, FAQ, Support, and Logout out of sidebars into top headers for both Admin and Dashboard layouts. Unified dashboard header to display on desktop and mobile, and removed notifications button for posters/drivers. Build: `npx tsc --noEmit` 0 errors, `npx eslint` 0 errors on all changed files, `npm run build` clean.
...
| Aug 30 | **Change Password + Logout (FEATURE-22):** 1) Created `POST /api/auth/change-password` route — `withAuth` guard, Zod validation (currentPassword min 1, newPassword min 8), 400 if user has no `passwordHash` (OAuth-only), 400 "Current password is incorrect" if bcrypt.compare fails, bcrypt.hash(newPassword, 10) + save, 200 on success; 2) Created shared `ProfileSidebar.tsx` — dark sidebar (`bg-[#0f1117]`), nav items: Edit Profile (plain) + Change Password (blue filled pill when active), Material Symbols icons; 3) Created shared `ChangePasswordForm.tsx` — 3 fields (current/new/confirm) with independent eye toggles, `react-hook-form` + `@hookform/resolvers/zod`, Zod cross-field validation (confirm matches new), `useChangePassword` mutation with success toast + form reset, inline error via `setError`; 4) Created `SettingsPageContent.tsx` shared client component, server-side `passwordHash` check via `verifyAccessToken` + DB query in per-role pages (`/(dashboard)/settings/page.tsx` + `/(admin)/admin/settings/page.tsx`); 5) Added Logout button to dashboard sidebar and AdminSidebar using `logoutUser()` (clears JWT cookies) + `signOut({ redirect, callbackUrl: '/login' })` per task spec; 6) Updated AdminSidebar Settings link from `/settings` to `/admin/settings`. All files: tsc 0 errors, ESLint 0 errors, build 60+ pages clean. | `src/app/api/auth/change-password/route.ts` (new), `src/components/profile/ProfileSidebar.tsx` (new), `src/components/profile/ChangePasswordForm.tsx` (new), `src/components/profile/SettingsPageContent.tsx` (new), `src/app/(dashboard)/settings/page.tsx` (new), `src/app/(admin)/admin/settings/page.tsx` (new), `src/app/(dashboard)/layout.tsx` (modified), `src/components/admin/AdminSidebar.tsx` (modified), `src/types/auth/auth.ts` (modified), `src/api/apis/auth/authApi.ts` (modified), `src/api/hooks/auth/authApi.ts` (modified) | None |
